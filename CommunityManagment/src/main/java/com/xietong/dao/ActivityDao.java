...
public interface ActivityDao {
     ...
     * @param id
     */
    @Update("update activity set astate=#{astate} where id=#{id}")
    public void updateStateById(int id,int astate);

    /**
     * 通过活动的id查询活动的状态
     * @param id
     */
    @Select("select astate from activity where id=#{id}")
     ...
     * @return 活动集合
     */
    @Select("select * from activity where astate=#{astate}")
    public List<Activity> findActivityByState(int astate);
}
